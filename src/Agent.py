...
def get_vectorstore(db_name: str):
    ...
    if db_name not in _vectorstore_cache:
        print(f"[INFO] ChromaDB 로드 중: {db_name}")
        embeddings = get_embeddings()  # 공유 임베딩 사용
        db_path = f"./Chroma_db/{db_name}"
        _vectorstore_cache[db_name] = Chroma(
...
def news_node(state: AgentState):
    ...
    res = llm.invoke(f"뉴스: {context}\n질문: {state['query']} 분석해줘. 반드시 뉴스 기사 제목과 뉴스 개시 날짜를 마지막에 밝혀야 돼.").content
    return {"response": res}
...
def stock_report_node(state: AgentState):
    ...
    res = llm.invoke(f"종목(회사) 리포트: {context}\n질문: {state['query']} 분석해줘. 반드시 리포트 출처와 리포트 개시 날짜를 마지막에 밝혀야 돼.").content
    return {"response": res}
...
def short_term_agent(state: AgentState):
    ...
    context = (
        ...
        search_db("News_chroma_db", state['query'], 3)
    )
...
def finalize_prediction(state: AgentState):
    ...
    history = "\n".join(state["debate_history"])
    res = llm.invoke(f"질문에 대한 토론 요약 및 최종 주가 예측 결론 도출 \n질문: {state['query']}:\n 토론 내용: {history} 반드시 리포트와 뉴스의 제목과 개시 날짜를 마지막에 출력해야돼. 출처를 밝혀.").content
    return {"response": res}
...
def run_chatbot():
    ...
    while True:
        ...
        query_with_date = f"[{today_str}] {user_input}"
        
        print(f"[INFO] 날짜가 추가된 쿼리: {query_with_date}")  # ✅ 디버깅용 출력

        # 초기 상태
        state = {
            "query": query_with_date,  # ✅ 수정: 날짜가 붙은 쿼리 사용
            "category": "",
            "sub_category": "",
            "debate_history": [],
            "debate_count": 0,
