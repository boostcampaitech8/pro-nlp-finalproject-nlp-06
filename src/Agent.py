...
def main_router(state: AgentState):
    ...
    system_prompt = """당신은 경제, 금융 챗봇을 위한 router입니다. 사용자의 입력에 대해 다음 3가지 중 하나로 분류하세요. 
    [chat] : 일반 대화, 인삿말, 농담 등 ,
    [rag] : 검색 엔진. 뉴스와 경제관련 리포트, 경제 용어, 금융 용어 등 참조가 필요한 질문.
    [prediction] : 주가 예측, 산업 예측 등 예측에 대한 질문. \
    용어에 대한 질문은 rag, 주가 예측은 prediction이라고 판단하세요. 다만 제공하는 기능에 대한 질문은 chat을 출력하세요"""
...
def vocab_node(state: AgentState):
    context = search_db("Vocab_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
    return {"results": [f"[용어 사전 검색 결과]\n{formatted_result}"]}

def news_node(state: AgentState):
    context = search_db("News_chroma_db", state['user_input'], k=5, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
    return {"results": [f"[뉴스 검색 결과]\n{formatted_result}"]}

def stock_report_node(state: AgentState):
    context = search_db("Company_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
    return {"results": [f"[종목 리포트 검색 결과]\n{formatted_result}"]}

def industry_report_node(state: AgentState):
    context = search_db("Industry_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
    return {"results": [f"[산업 리포트 검색 결과]\n{formatted_result}"]}

def market_report_node(state: AgentState):
    context = search_db("MarketConditions_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
    return {"results": [f"[시황 리포트 검색 결과]\n{formatted_result}"]}

def economy_report_node(state: AgentState):
    context = search_db("Economy_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    formatted_result = "\n\n".join(context)
...
def research_agent_node(state: AgentState):
    ...
    company_context = search_db("Company_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    industry_context = search_db("Industry_report_chroma_db", state['user_input'], k=3, min_similarity = 0.3)
    news_context = search_db("News_chroma_db", state['user_input'], k=5, min_similarity = 0.3)
...
6. Quant Agent에 대한 예측 요약을 먼저 제시합니다"""
...
5. **표 출력 자제** : 표 출력을 자제하고 보기 쉽게 작성하세요"""
    user_prompt = f"질문: {state['query']}\n\n분석 결과들:\n{combined_context}"
